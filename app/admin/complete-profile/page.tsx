...
import { CreateAdminProfile } from "./action";
...
export default function AdminCompleteProfile(){
  ...
  function onSubmit(data: z.infer<typeof AdminProfileSchema>) {
    startTransition(async () => {
      const { data: result, error } = await tryCatch(
        CreateAdminProfile(data),
      );
      ...
      <Card className="w-full max-w-4xl shadow-lg sm:shadow-xl border rounded-2xl">
        ...
        <CardHeader className="space-y-4 text-center px-4 sm:px-8 pt-6 sm:pt-8">
          ...
          <CardDescription className="text-sm sm:text-base text-muted-foreground max-w-md mx-auto">
            Set up your Admin profile to start managing
            academic content, and users on learnosi.
          </CardDescription>
        </CardHeader>
